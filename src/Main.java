...
// GamePanel.java
// Arshdeep Sidhu
// This is the game panel, where the real graphics go down. This panel is where
...
class GamePanel extends JPanel implements MouseMotionListener, MouseListener {
    ...
    public void spawn() {
        if (warrior.contains(mx, my)
                && (currentStats.level == 1 || currentStats.level == 2 || currentStats.level == 3)) {
    ...
    // if(gridpointsx.contains(x)&& gridpointsy.contains(y) &&
    // gridpointsx.indexOf(x)==gridpointsy.indexOf(y)){
    ...
    // if(diffx>=Math.abs(x-i) && diffy>=Math.abs(y-j) &&
    // gridpointsx.indexOf(i)==gridpointsy.indexOf(j)){
    // diffx = Math.abs(x-i);
    // diffy = Math.abs(y-j);
    ...
    public void paintComponent(Graphics g) {
        ...
        // g.drawImage(attackButton,250,300,this); may need to remove this one
    ...
    public void mousePressed(MouseEvent e) {
        ...
        my = e.getY();
        click = true;
        if (buildingStats == true && !spawnButton.contains(mx, my) && !warrior.contains(mx, my)
                && !archer.contains(mx, my) && !wizard.contains(mx, my)) {
            buildingStats = false;
            spawnMenu = false;
            st.setVisible(false);
        }
